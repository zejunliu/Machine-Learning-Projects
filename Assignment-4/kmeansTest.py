...
def transform_image(image, code_vectors):
    ...
    new_image =[[[0 for i in range(3)]for j in range(512)] for k in range(512)]
    for i in range(len(image)):
        for j in range(len(image[0])):
            best_m = -1
            min_distance = 2147483647
            for m in range(len(code_vectors)):
                distance = 0
                distance += np.square(np.linalg.norm(np.array(image[i][j]) - np.array(code_vectors[m])))   
                if distance < min_distance:
                    min_distance = distance
                    best_m = m
            new_image[i][j] = code_vectors[best_m]
