...
def pca(X = np.array([]), no_dims = 50):
    ...
    eig_vals, eig_vects = np.linalg.eig(np.mat(np.cov(X, rowvar=0)))
    eig_val_index = np.argsort(eig_vals)
    eig_val_index = eig_val_index[:-(no_dims + 1): -1]
    reg_eig_vects = eig_vects[:, eig_val_index]
    Y = np.array(np.matmul(X, reg_eig_vects))
    M = np.array(reg_eig_vects)
    return Y, M
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    for i in range(D):
        error += np.square(orig[i] - decompressed[i])
    error /= D
    """TODO: write your code here"""
