...
def objective_function(X, y, w, lamb):
    ...
    for i in range(len(xw)):
        ...
        if temp <= 0:
            ...
            sum += temp
    avgsum = sum * 1.0 / len(xw)

    wsum = 0
    for i in range(len(w)):
        wsum += np.power(w[i, ],2)

    res = wsum * lamb * 1.0 / 2 + avgsum
    return res[0]
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        temp_w = np.zeros(D)
        for i in A_t_plus:
            temp_w += Xtrain[i] * ytrain[i]
        temp_w = temp_w * 1.0 / (iter *  lamb) / k
        w_half = (1 - 1.0 / (iter *  lamb) * lamb) * w + np.transpose(temp_w).reshape(D,1)
        
        temp_w_half = 0
        for i in range(len(w_half)):
            temp_w_half += np.power(w_half[i, ], 2)

        temp_w_one = 1.0 / np.sqrt(lamb) /np.sqrt(temp_w_half)
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    for i in range(len(y_pred)):
        ...
        if y_pred[i, ] == ytest[i]:
            num_correct += 1
    return num_correct * 1.0/len(y_pred)
